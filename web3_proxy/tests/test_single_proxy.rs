...
async fn test_single_proxy_stats_add_up() {
    ...
    let user_1_login = create_user(&x, &r, &user_1_wallet, None).await;
    let admin_login = create_user_as_admin(&x, &db, &r, &admin_wallet).await;

    // Load up balances
    admin_increase_balance(&x, &r, &admin_login, &user_0_wallet, Decimal::from(1000)).await;
    admin_increase_balance(&x, &r, &admin_login, &user_1_wallet, Decimal::from(2000)).await;

    let user_0_balance = user_get_balance(&x, &r, &user_0_login).await;
    let user_1_balance = user_get_balance(&x, &r, &user_1_login).await;
    ...
    let user_0_secret_key = user_get_first_rpc_key(&x, &r, &user_0_login)
        .await
        .secret_key;

    let proxy_0_user_0_provider =
        create_provider_for_user(x.proxy_provider.url(), &user_0_secret_key).await;
    ...
    // Get the balance
    let user_0_balance_post = user_get_balance(&x, &r, &user_0_login).await;
    ...
    assert_eq!(user_0_balance_post.total_frontend_requests, number_requests);
    assert_eq!(
